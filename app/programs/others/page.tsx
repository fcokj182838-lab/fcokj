...
export const metadata: Metadata = {
  title: "기타사업 · 법인사업",
  description:
    "사단법인 외국인과 동행 기타사업 안내입니다. 지역 소통, 문화축제, 검진, 스포츠 대회 등 외국인의 조기적응과 다문화 정착을 위한 다양한 사업을 운영합니다.",
};
...
const OTHER_PROGRAMS: OtherProgram[] = [
  { code: "01", title: "지역리더와 소통마당", english: "Local Leader Forum", category: "소통", accent: "from-[var(--color-terracotta)]/30 via-[var(--color-terracotta-soft)]/15 to-[var(--color-cream)]" },
  { code: "02", title: "세계인의 문화축제", english: "Global Culture Festival", category: "축제", accent: "from-[var(--color-sage)]/35 via-[var(--color-cream)] to-[var(--color-cream-dark)]" },
  { code: "03", title: "한국어능력시험 모의고사", english: "TOPIK Mock Test", category: "교육지원", accent: "from-[var(--color-ink)]/20 via-[var(--color-sage)]/15 to-[var(--color-ivory)]" },
  { code: "04", title: "캄보디아 쫄츠남 축제", english: "Choul Chnam Festival", category: "공동체행사", accent: "from-[var(--color-terracotta-soft)]/30 via-[var(--color-cream)] to-[var(--color-sage)]/15" },
  { code: "05", title: "주말 원동기 면허시험", english: "Motorbike License", category: "생활지원", accent: "from-[var(--color-cream-dark)] via-[var(--color-cream)] to-[var(--color-terracotta-soft)]/15" },
  { code: "06", title: "스리랑카 체육대회", english: "Sri Lanka Sports Day", category: "스포츠", accent: "from-[var(--color-terracotta)]/25 via-[var(--color-cream)] to-[var(--color-ivory)]" },
  { code: "07", title: "무료 결핵 검진", english: "Free TB Checkup", category: "의료지원", accent: "from-[var(--color-sage)]/25 via-[var(--color-cream)] to-[var(--color-cream-dark)]" },
  { code: "08", title: "스리랑카·방글라데시 크리킷 대회", english: "Cricket Tournament", category: "스포츠", accent: "from-[var(--color-ink)]/15 via-[var(--color-cream)] to-[var(--color-sage)]/15" },
  { code: "09", title: "외국인 스포츠팀 운영", english: "Sports Team", category: "스포츠", accent: "from-[var(--color-terracotta-soft)]/20 via-[var(--color-cream)] to-[var(--color-cream-dark)]" },
  { code: "10", title: "국제 축구대회", english: "International Football Cup", category: "스포츠", accent: "from-[var(--color-terracotta)]/22 via-[var(--color-cream)] to-[var(--color-ivory)]" },
];
...
const PROGRAM_SUB_PAGES = [
  ...
  { label: "기타사업", href: "/programs/others", english: "Others", current: true },
];
    ...
    <section className="relative overflow-hidden">
      ...
      <div className="mx-auto max-w-[1280px] px-6 pb-20 pt-16 lg:px-10 lg:pb-28 lg:pt-24">
        <nav aria-label="현재 위치" className="flex items-center gap-3 border-b border-[var(--color-line)] pb-6 animate-fade-in">
          ...
          <span className="h-px w-6 bg-[var(--color-line)]" />
          <span className="font-[var(--font-display)] text-[11px] uppercase tracking-[0.3em] text-[var(--color-terracotta)]">Others</span>
        </nav>
        <div className="mt-12 grid grid-cols-1 gap-8 lg:mt-16 lg:grid-cols-12 lg:gap-12">
          <div className="lg:col-span-8">
            ...
            <h1 className="mt-6 font-[var(--font-serif)] text-[44px] font-medium leading-[1.05] tracking-[-0.02em] text-[var(--color-ink)] animate-fade-up sm:text-[60px] lg:text-[80px]">
              ...
              <span className="block"><em className="font-[var(--font-display)] not-italic text-[var(--color-terracotta)]">기타사업</em></span>
            </h1>
          </div>
        </div>
      </div>
    ...
    <section className="relative bg-[var(--color-ivory)] py-24 lg:py-32">
      <div className="mx-auto max-w-[1100px] px-6 lg:px-10">
        <blockquote className="font-[var(--font-serif)] text-[24px] leading-[1.6] tracking-[-0.01em] text-[var(--color-ink)] sm:text-[30px] md:text-[36px] md:leading-[1.5]">
          ...
          외국인에게 필요한 다양한 사업을 추진하여 지역사회 조기 적응을 돕고, 내국인과 외국인이 함께
          서로를 이해하는 축제를 운영하고 있습니다.
        </blockquote>
        <p className="mt-8 max-w-3xl text-[15px] leading-[1.9] text-[var(--color-ink-soft)]">
          국가별 공동체 행사 지원을 통해 다문화가 지역사회에 자연스럽게 정착할 수 있도록 실질적인
          프로그램을 꾸준히 운영합니다.
        </p>
        <div className="mt-10 grid grid-cols-3 gap-4 md:gap-6">
          <Stat number="10" label="기타사업 프로그램" code="PROGRAMS" />
          <Stat number="다문화 정착" label="핵심 목표" code="MISSION" />
          <Stat number="연중 운영" label="상시 추진" code="SCHEDULE" />
        </div>
      </div>
    </section>
  );
      ...
      <div className="mx-auto max-w-[1280px] px-6 lg:px-10">
        <div className="grid grid-cols-1 items-end gap-6 border-b border-[var(--color-line)] pb-10 md:grid-cols-12">
          <div className="md:col-span-7">
            <p className="font-[var(--font-display)] text-[11px] uppercase tracking-[0.3em] text-[var(--color-terracotta)]">─ Other Programs</p>
            <h2 className="mt-4 font-[var(--font-serif)] text-3xl font-medium leading-tight tracking-[-0.02em] text-[var(--color-ink)] md:text-5xl lg:text-6xl">
              열 가지
              <br />
              <em className="font-[var(--font-display)] not-italic text-[var(--color-terracotta)]">기타사업</em>
            </h2>
          </div>
        </div>
    ...
    <section className="relative overflow-hidden bg-[var(--color-ink)] py-20 text-[var(--color-cream)]">
      <div className="relative mx-auto max-w-[1100px] px-6 lg:px-10">
        <h2 className="font-[var(--font-serif)] text-3xl md:text-5xl">
          기타사업 문의는 <em className="font-[var(--font-display)] not-italic text-[var(--color-terracotta-soft)]">언제든</em> 가능합니다.
        </h2>
